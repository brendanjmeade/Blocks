...
if sum(Segment.patchTog) > 0 & ~isempty(Patches.c) % if patches are involved at all
   ...
   [Wtri, Wseg]                                  = SmoothEdges(share, Patches, Segment, Partials.slip);                 
   Wtri                                          = Wtri(:, colkeep); % eliminate the entries corresponding to empty partials
   Wtri(3:3:end, :)                              = []; % eliminate vertical velocity components
   Wseg(3:3:end, :)                              = [];
   ...
   for i = 1:numel(Patches.nEl)
      distScale                                  = mean(dists(dists(s(i)+1:s(i+1), :) > 0));
      beta(2*(s(i)+1)-1:2*s(i+1))                = distScale^2*Command.triSmooth(i);
   end
   [Wseg, Wtri]                                  = deal(repmat(beta, 1, size(Wseg, 2)).*Wseg, repmat(beta, 1, size(Wtri, 2)).*Wtri);
else
...
w                                                = [w ; ones(size(Wtri, 1), 1)]; % add the triangular smoothing vector
w                                                = [w ; ones(size(Wcons, 1), 1)]; % add the triangular kinematic consistency vector
w                                                = [w ; ones(size(Ztri, 1), 1)]; % add the zero edge vector
...
if isempty(Command.triCons)
   if Command.strainMethod ~= 4
      omegaEst                                   = Model.covariance*R'*W*d;
   elseif Command.strainMethod == 4
      % Call the simulated annealing solver to estimate the location of the reference coordinates (strain rate)
      disp('Strain method 4')
      ...
      GLOBAL.W       = full(W);
      GLOBAL.d       = full(d);
      GLOBAL.strainPadding = strainPadding;
%       [strainRefCoordsEst, fval] = simulannealbnd(@StrainRefCoordsEstFun, [Model.lonStrain(:) ; Model.latStrain(:)]);
      if ~isempty(strfind(version, 'R2008'))
         psoptions = psoptimset('UseParallel', 'always')
      end
      [strainRefCoordsEst, fval] = patternsearch(@StrainRefCoordsEstFun, [Model.lonStrain(:) ; Model.latStrain(:)]);
      
      Model.lonStrainSa = strainRefCoordsEst(1:end/2);
      Model.latStrainSa = strainRefCoordsEst(end/2+1:end);

      % Rebuild strain partials with estimated best coordinates
      [Partials.strain, strainBlockIdx,...
       Model.lonStrain, Model.latStrain]         = GetStrainReferencePartials(Block, Station, Segment, Model.lonStrainSa, Model.latStrainSa);
   
      % Rebuild R with the new strain partials based on the current guess for the reference coordinates
      strainIdx = size(R, 2) - size(strainPadding, 2); % determine width of the R matrix not including strains
      R(:, strainIdx + 1:end) = [Partials.strain; strainPadding];
      Model.covariance                           = inv(R'*W*R);
      omegaEst                                   = Model.covariance*R'*W*d;
   end
else
...
%     Model.latStrain(strainBlockIdx)            = 
   ...
   case 4 % directed forward search method, same parameter extraction as the 3 parameter estimation
      dEst                                       = omegaEstStrain;
      m1                                         = dEst(1:3:end);
      m2                                         = dEst(2:3:end);
      m3                                         = dEst(3:3:end);
      ...
      Model.eLonLatSig(strainBlockIdx)           = m2Sig;
      Model.eLatLatSig(strainBlockIdx)           = m3Sig;
      % Reference coordinates estimated by the directed search were stored above.
end

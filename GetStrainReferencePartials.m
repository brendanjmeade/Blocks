function [G, strainBlockIdx, cLon, cLat]    = GetStrainReferencePartials(Block, Station, Segment, lon0Vec, lat0Vec)
% Calculate strain partial derivatives
nStations                                   = numel(Station.lon);
nBlocks	                                   = numel(Block.interiorLon);
G                                           = zeros(2*nStations, 3*nBlocks);
...
%       lat0                                  = mean(latVec(:));
%       lon0                                  = mean(lonVec(:));
      lat0                                  = lat0Vec(Station.blockLabel(iStation));
      lon0                                  = lon0Vec(Station.blockLabel(iStation));

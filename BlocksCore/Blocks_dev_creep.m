function Blocks_dev_creep(commandFile)
...
if sum(Segment.patchTog) > 0 & ~isempty(Patches.c) % if patches are involved at all
   ...
   % apply apriori patch slip rates ELE 11/09
%   load(Command.patchFileNames, 'creepdist','err','k','ap_T');
   load(Command.patchFileNames, 'rate','rateSig','idx','ap_T');
   Patches.creep                                  = [];
   Patches.creep                                  = -rate;
   Patches.err                                    = [];
   Patches.err                                    = rateSig;
...
triblockslip                                     = triblockslip(colkeep,:);
triblockslip                                     = triblockslip(2*k-1,:);
keyboard
...
R                                                = [R ; [triblockslip -ap_T(:,colkeep)]]; % Add creep rate constraints
...
d                                                = [d ; nonzeros(Patches.creep)]; % Add creep rate constraints
...
w                                                = [w ; 1e-3*nonzeros(Patches.err)]; % Add creep rate constraints
w                                                = [w ; beta.*ones(size(Wtri, 1), 1)]; % add the triangular smoothing vector

function Blocks(commandFile, ld)
...
%   Wtri                                          = MakeTriSmooth(share, dists);
   [Wtri, Wseg, beta, Wsc, Wtc]                  = SmoothEdges(share, Patches, Segment, Partials.slip, Command);                 
...
%   [Wseg, Wtri]                                  = deal(repmat(beta', size(Wseg, 1), 1).*Wseg, repmat(beta', size(Wtri, 1), 1).*Wtri);
   ...
   [Wseg, Wtri, Wcons, Wsc, Wtc, Wconnect, beta, Ztri]     = deal([]);
end
...
rconn                                            = size(Wsc, 1); % Triangle-segment connection constraint rows
...
% Determine indices
rnum                                             = [rsta rbcons rscons rtriw rtriz rconn];
...
R(rows{6, 1}, cols{6, 1})                        = Wsc;
R(rows{6, 2}, cols{6, 2})                        = Wtc;
...
d                                                = [d ; sparse(size(Wsc, 1), 1)]; % Add zeros for segment-tri. constraints
...
w                                                = [w ; 1e5*ones(size(Wsc, 1), 1)]; % add the seg.-tri. connection vector

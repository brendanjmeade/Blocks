...
% Assign unit uncertainties if specified
if strmatch(Command.unitSigmas, 'yes')
   [Station.eastSig, Station.northSig]           = ones(numel(Station.lon), 1);
end
% Read SAR data
Sar                                              = struct('lon', zeros(0, 1), 'lat', zeros(0, 1), 'data', zeros(0, 1), 'look_vec', zeros(0, 1), 'ref_pt', zeros(0, 1), 'dataSig', zeros(0, 1));
if ~isempty(Command.sarFileName)
   Sar                                           = ReadSar(Command.sarFileName);
   % Assign uniform uncertainties if not specified
   if ~isfield(Sar, 'dataSig')
      Sar.dataSig                                = Command.sarSig.*ones(size(Sar.lon));
   end   
end

% Read segment file
Segment                                          = ReadSegmentTri(Command.segFileName); 
% Don't allow buried faults
Segment.bDep                                     = zeros(size(Segment.bDep));
% Reorder segment endpoints in a consistent fashion; west is always first
Segment                                          = OrderEndpoints(Segment); 
% Read block file
Block                                            = ReadBlock(Command.blockFileName); 
% Read triangulated patches
Patches                                          = struct('c', [], 'v', []);
...
% Read patch creep files, if applicable
if ~isempty(Command.patchCreepTog)
   tnel                                          = cumsum([0 Patches.nEl]);
   Creep                                         = struct('rate', [], 'rateSig', [], 'idx', [], 'ap_T', zeros(0, 3*tnel(end)));
   creepidx                                      = [];
   spaces                                        = [0 findstr(Command.patchFileNames, ' ') length(Command.patchFileNames)+1];
   for i = 1:length(Command.patchCreepTog)
      filename                                   = Command.patchFileNames(spaces(i)+1:spaces(i+1)-1);
      if Command.patchCreepTog == 1 & filename(end-3:end) == '.mat'
         load(filename, 'rate', 'rateSig', 'idx', 'ap_T');
         Creep.rate                              = [Creep.rate; -rate];
         Creep.rateSig                           = [Creep.rateSig; rateSig];
         big_ap_T                                = zeros(size(ap_T, 1), 3*tnel(end));
         big_ap_T(:, tnel(i)+(1:3*Patches.nEl(i))) = ap_T;
         Creep.ap_T                              = [Creep.ap_T; big_ap_T];
         Creep.idx                               = [Creep.idx; tnel(i) + idx]; 
      end
   end
end
...
if isempty(Sar.lon)
   ...
   [Partials.selastic, Partials.stri]            = deal([]);
else
   [Partials.elastic, Partials.selastic,...
    Partials.tri, Partials.stri,...
    Partials.trizeros, Partials.tristrikes]      = CheckExistingKernelsSar(Command, Segment, Station, Sar, Patches);
end
...
if sum(Segment.patchTog) > 0 & ~isempty(Patches.c) % if patches are involved at all
   ...
   if sum(Command.patchCreepTog) > 0
      creepblockslip                             = Partials.trislip(colkeep, :);
      % *** This array is trimmed to keep only strike-slip creep components; dip- and/or tensile-slip would have to be added later
      creepblockslip                             = creepblockslip(2*Creep.idx-1, :);
   else
      creepblockslip                             = zeros(0, size(Partials.tri, 2));
   end
...
switch Command.strainMethod
   case 1 % use the block centroid
      [Partials.strain, strainBlockIdx,...
       Model.lonStrain, Model.latStrain,...
       toDelete]                                 = GetStrainCentroidPartials(Block, Station, Segment);
...
% Calculate the SAR partials; these will eventually move up to near their GPS counterparts, and existing elastics will be checked 
if ~isempty(Sar.lon)
   fprintf('\n  Calculating SAR partials...')
   Partials.srotation                            = GetSarRotationPartials(Sar, Block, Sar.look_vec);
   if isempty(Partials.selastic)
      Partials.selastic                          = GetSarElasticPartials(Segment, Sar, Sar.look_vec);
   end
   if isempty(Partials.stri)
      Partials.stri                              = GetSarTriPartials(Patches, Sar, Sar.look_vec);
   end
   if strcmp(Command.saveKernels, 'yes') == 1;
      save('tempkernels.mat', '-struct', 'Partials', 'selastic', 'stri', '-mat', '-append');
   end
   Partials.stri                                 = Partials.stri(:, colkeep);
%   Partials.sstrain                              = GetSarStrainCentroidPartials(Block, Sar, Segment, Sar.look_vec);
%   Partials.sramp                                = GetSarLinearRampPartials(Sar);
%   Partials.sramp                                = GetSarCubicRampPartials(Sar);
   Partials.sramp                                = GetSarRampPartials(Sar);
   Partials.sstrain                              = GetSarStrainCentroidPartials(Block, Sar, Segment, Sar.look_vec, toDelete);
else
   Partials.srotation                            = zeros(0, size(Partials.rotation, 2));
   Partials.selastic                             = zeros(0, size(Partials.elastic, 2));
   Partials.stri                                 = zeros(0, size(Partials.tri, 2));
   Partials.sstrain                              = zeros(0, size(Partials.strain, 2));
   Partials.sramp                                = zeros(0, 0);
end
%Partials.sstrain = zeros(0, size(Partials.strain, 2));
szsr = size(Partials.sramp);
%Partials.sstrain = zeros(szsr(1), size(Partials.strain, 2));
...
cramp                                            = size(Partials.sramp, 2); % Ramp columns
cstrain                                          = size(Partials.strain, 2); % Strain columns

% Determine indices
rnum                                             = [rsta rsar rbcons rscons rcreep rtriw rtriz];
cnum                                             = [cblock ctri cramp cstrain];
ridx                                             = cumsum([0 rnum]);
cidx                                             = cumsum([0 cnum]);
...
R(rows{1, 4}, cols{1, 4})                        = Partials.strain;
R(rows{2, 1}, cols{2, 1})                        = Partials.srotation - Partials.selastic * Partials.slip;
R(rows{2, 2}, cols{2, 2})                        = -Partials.stri;
R(rows{2, 3}, cols{2, 3})                        = Partials.sramp;
R(rows{2, 4}, cols{2, 4})                        = Partials.sstrain;
R(rows{3, 1}, cols{3, 1})                        = Partials.blockCon;
R(rows{4, 1}, cols{4, 1})                        = Partials.slipCon;
R(rows{5, 1}, cols{5, 1})                        = creepblockslip;
R(rows{5, 2}, cols{5, 2})                        = -Creep.ap_T(:, colkeep);
R(rows{6, 2}, cols{6, 2})                        = Wtri;
R(rows{7, 2}, cols{7, 2})                        = Ztri;
...
%R                                                = [R sparse(size(R, 1), szsr(2))]; % add padding for SAR ramp partials
%R                                                = [R ; [(Partials.srotation - Partials.selastic * Partials.slip) -Partials.stri Partials.sramp]]; % Add SAR partials
%R                                                = [R ; [Partials.blockCon sparse(size(Partials.blockCon, 1), sztri(2)+szsr(2))]]; % Add block motion constraints
%R                                                = [R ; [Partials.slipCon sparse(size(Partials.slipCon, 1), sztri(2)+szsr(2))]]; % Add slip rate constraints
%R                                                = [R ; [Wseg, Wtri, sparse(size(Wtri, 1), szsr(2))]]; % Add triangular smoothing matrix
%R                                                = [R ; [Wcons, sparse(size(Wcons, 1), szsr(2))]]; % Add kinematic constraints
%R                                                = [R ; [sparse(size(Ztri, 1), szrot(2)) Ztri sparse(size(Ztri, 1), szsr(2))]]; % Add edge slip constraints
%strainPadding                                    = zeros(size(R, 1)-size(Partials.strain, 1)-size(Partials.sstrain, 1), size(Partials.strain, 2));
%R                                                = [R [Partials.strain; Partials.sstrain; strainPadding]];
%toc
%keyboard
fprintf('done.\n')
...
w                                                = w(:);
w                                                = w(rowkeep);
w                                                = [w ; Command.sarWgt*1./Sar.dataSig.^2]; % Add SAR weight and uncertainties
...
w                                                = [w ; Command.creepConWgt.*(nonzeros(Creep.rateSig))]; % Add creep rate constraints
%w                                                = [w ; Command.creepConWgt.*1./(nonzeros(Creep.rateSig)).^2]; % Add creep rate constraints
...
omegaEstSarRamp                                  = omegaEst(szslip(2) + sztri(2) + (1:szsr(2))); % SAR ramp parameters
omegaEstStrain                                   = omegaEst(szslip(2) + sztri(2) + szsr(2) + 1:end); % strain parameters
...
Model.covarianceSarRamp                          = Model.covariance(szslip(2) + sztri(2) + (1:szsr(2)), szslip(2) + sztri(2) + (1:szsr(2)));
Model.covarianceStrain                           = Model.covariance(szslip(2) + sztri(2) + szsr(2) + 1:end, szslip(2) + sztri(2) + szsr(2) + 1:end);
...
[Model.lonEulerSig,...
 ...
 Model.lonLatCorr,...
 Model.lonRateCorr,...
 Model.latRateCorr]                              = OmegaSigToEulerSig(omegaEstRot(1:3:end), omegaEstRot(2:3:end), omegaEstRot(3:3:end), Model.covarianceRot);
...
% SAR predictions
Model.Sar                                        = (Partials.srotation-Partials.selastic * Partials.slip)*omegaEstRot + Partials.sramp*omegaEstSarRamp;
Model.SarDef                                     = (Partials.selastic * Partials.slip)*omegaEstRot;
Model.SarRot                                     = Partials.srotation*omegaEstRot;
Model.SarRamp                                    = Partials.sramp*omegaEstSarRamp;
...
[Model.SarTri]                                   = zeros(size(Sar.lon, 1), 1); 
...
if numel(omegaEstTriSlip) > 0
   ...
   Model.SarTri                                  = Partials.stri*omegaEstTriSlip;
...
if Command.strainMethod > 0
   ...
   Model.SarStrain                               = Partials.sstrain*omegaEstStrain;
%   Model.SarStrain                               = zeros(size(Model.Sar));
   ...
   Model.SarStrain                               = zeros(size(Sar.lon));
end   
